...
# You might need Root privileges for building image locally
	docker-compose run application python main.py --load-config '{"load_type":"GenericPySparkLoad", "options": {"path": "/app/input/sample.csv", "format": "csv", "header": "true"} }'  --sink-config '{"sink_type": "JDBCSink", "options":{"conn_uri": "amRiYzpwb3N0Z3Jlc3FsOi8vcG9zdGdyZXM6NTQzMi9qb2JzaXR5P3VzZXI9bmFuaXZpYWEmcGFzc3dvcmQ9MTIzNDU2", "table": "trips"} }' --pipeline '{"pipeline_name": "Trips", "options":{}}'


run_aggregation:
# You might need Root privileges for building image locally
	docker-compose run application python main.py  --load-config '{"load_type":"JDBCLoad", "options":{"conn_uri": "amRiYzpwb3N0Z3Jlc3FsOi8vcG9zdGdyZXM6NTQzMi9qb2JzaXR5P3VzZXI9bmFuaXZpYWEmcGFzc3dvcmQ9MTIzNDU2", "table": "trips"}  }' --sink-config '{"sink_type": "GenericPySparkSink", "options":{"format":"json", "path":"/app/output/aggregated", "mode":"overwrite"}}' --pipeline '{"pipeline_name": "TripsAggregated", "options":{"region":"Turin", "p1":[7.54, 44], "p2":[7.70, 46]}}'
